...
docker run -v $(pwd):/app -p 3000:3000 ghcr.io/dufeut/mik
```
...
services:
  mik:
    image: ghcr.io/dufeut/mik:latest
    ports:
      - "3000:3000"
    volumes:
      - ./:/app
...
```dockerfile
FROM ghcr.io/dufeut/mik:latest
...
```bash
docker run --rm -v $(pwd):/app --entrypoint="" ghcr.io/dufeut/mik:latest /mik run
```
