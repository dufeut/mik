...
:::note[Daemon Configuration]
For daemon-level settings (port, service toggles), see the separate `~/.mik/daemon.toml` file documented in the [Daemon Guide](/guides/daemon#daemon-configuration).
:::
