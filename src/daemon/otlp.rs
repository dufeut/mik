...
/// Initialize logging with OTLP tracing layer.
///
...
///
/// ```rust,ignore
/// use mik::daemon::otlp::{init_with_otlp, OtlpConfig};
///
/// init_with_otlp(OtlpConfig::new("http://jaeger:4317"))?;
///
/// // Now tracing spans will be exported
...
pub fn init_with_otlp(config: OtlpConfig) -> Result<(), OtlpError> {
    ...
    let tracer = provider.tracer("mik");
    ...
    // Build filter from RUST_LOG env or default to INFO
...
mod tests {
    ...
    fn test_otlp_config_default() {
        ...
        assert_eq!(config.service_name, "mik");
    }

    #[test]
    fn test_otlp_config_builder() {
        let config = OtlpConfig::new("http://jaeger:4317").with_service_name("my-service");
